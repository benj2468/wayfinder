...
impl blue::BleAdvertiser for ForeignAdvertiser {
    async fn advertise(&self, fragment: &[u8]) -> Result<(), LinkError> {
        self.inner
            .advertise(fragment.to_vec())
            .await
            .map_err(|_| LinkError::TransmitFailed)
    }
}
...
pub struct MeshNode {
    // Not yet driven by anything outside `#[cfg(test)]`'s `recv_for_test` —
    // a later phase hands this to `wayfinder-driver::Driver` as a `LinkT`.
    // Kept (rather than only storing `sink`) because that's the field this
    // crate's whole reason for existing needs next.
...
impl MeshNode {
    ...
    /// raised as an error, matching how a malformed on-air fragment is
    /// dropped rather than erroring (`blue`'s `recv`).
    pub fn submit_report(&self, addr: Vec<u8>, rssi: Option<i16>, fragment: Vec<u8>) {
        let addr_len = addr.len();
        let Ok(addr) = <[u8; 6]>::try_from(addr) else {
            tracing::trace!(addr_len, "drop: malformed BLE address");
            return;
        };
...
/// Owned snapshot of a `wayfinder::link::LinkT::recv` result, sidestepping
/// the borrowed [`wayfinder::link::Received`]'s lifetime so tests can hold
/// it across `await` points without also holding the link's lock.
#[cfg(test)]
struct ReceivedForTest {
    dst: wayfinder::interfaces::frame::Mac,
    protocol: u16,
    payload: Vec<u8>,
    rssi_dbm: Option<i16>,
}
...
#[cfg(test)]
impl MeshNode {
    /// Drive this node's [`wayfinder::link::LinkT::recv`] directly — not
    /// part of the UniFFI surface (Kotlin has no use for the internal
    /// `LinkFrame` type yet, pending the `Driver` wiring a later phase
    /// adds), but needed here to prove `submit_report` actually reaches the
    /// reassembler.
    async fn recv_for_test(&self) -> Result<ReceivedForTest, LinkError> {
        use wayfinder::link::LinkT as _;

        let mut link = self.link.lock().await;
        let received = link.recv().await?;
        Ok(ReceivedForTest {
            dst: received.frame.dst,
            protocol: received.frame.protocol.get(),
            payload: received.frame.payload.to_vec(),
            rssi_dbm: received.metrics.rssi_dbm,
        })
    }
}
...
mod tests {
    ...
    async fn submit_report_delivers_a_fragment_sent_by_another_blue_link() {
        // Simulate a peer (any `blue` backend) sending a frame: capture the
        // exact on-air fragment bytes via a recording `BleAdvertiser`, the
        // same shape a real Android scan callback would report.
        ...
        sender
            ...
            .await
            .unwrap();
        let fragments = sent.lock().unwrap().clone();
        ...
        let received = node.recv_for_test().await.unwrap();
        assert_eq!(received.dst, mac(2));
        assert_eq!(received.protocol, 0x9);
        assert_eq!(received.payload, payload);
        assert_eq!(received.rssi_dbm, Some(-50));
    }

    #[tokio::test]
    async fn submit_report_drops_malformed_address_without_panicking() {
        let node = MeshNode::new(Arc::new(FakePlatformAdvertiser::default()));
